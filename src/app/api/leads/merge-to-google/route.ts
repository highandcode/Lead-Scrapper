...
// categories (or every category when allCategories is set — optionally
// narrowed to a created_at date range) as data_source="google_search" so
// they show up under Google Leads, skipping any that already have an
// equivalent there (place_id, phone, or name+city match).
export async function POST(request: NextRequest) {
  const { response } = await requireApiPermission("leadsWrite");
  if (response) return response;

  try {
    const { categories, allCategories, dateFrom, dateTo } = (await request.json()) as {
      categories?: string[];
    ...
    if (!allCategories && (!Array.isArray(categories) || categories.length === 0)) {
    ...
    // allCategories also picks up leads with no category set at all — a
    // per-category .in() filter never could, since it can't match null.
    if (!allCategories) candidateQuery = candidateQuery.in("category", categories as string[]);
