...
interface JustDialPanelProps {
  onSearchComplete?: (count: number) => void;
}

export default function JustDialPanel({ onSearchComplete }: JustDialPanelProps) {
  ...
  async function handleSearch() {
    ...
    const searchToast = toast.loading(`Scraping JustDial for "${query}"...`);

    try {
      const response = await fetch("/api/scrape/justdial", {
        method: "POST",
    ...
    >
      ...
      <div className="p-6 border-b border-white/8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-foreground">Find Clinic Leads via JustDial</h2>
            <p className="text-xs text-muted-foreground">Scrape JustDial listings — more active local clinics</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-5">
          ...
          {showAdvanced && (
            ...
            >
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="space-y-2">
                  <label className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground uppercase tracking-wider">
                    <Star className="w-3 h-3" /> Min JustDial Rating
                  </label>
        ...
        {canSearch && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex items-center gap-2 p-3 rounded-lg bg-orange-500/5 border border-orange-500/15"
          >
            <Sparkles className="w-3.5 h-3.5 text-orange-400 flex-shrink-0" />
            <p className="text-xs text-orange-300">
              Will search JustDial: <span className="font-semibold">"{effectiveNiche} {effectiveCity}"</span> — up to {limit} results
            </p>
          </motion.div>
        )}
        ...
        {/* Submit */}
        <Button
          className="w-full bg-gradient-to-r from-orange-500 to-amber-400 hover:from-orange-600 hover:to-amber-500 text-white shadow-lg shadow-orange-500/20 h-11"
          ...
          {lastResult && (
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">for "{lastResult.query}" on JustDial</p>
              </div>
              <a href="/leads" className="text-xs text-orange-400 hover:underline">
                View leads →
              </a>
            </motion.div>
          )}
